...
mysqlConnection.connect( err => {
  if (!err) {
    ...
    io.on('connection', (socket) => {
      ...
      socket.on('create_discount', (data) =>{
        console.log("create_discount ::", data)
        socket.broadcast.emit('create_discount_complete', {
      ...
      socket.on('add_noti_testvideo_admin', (data) =>{
        ...
        mysqlConnection.query(`insert into home_notifications (user_id, title, text, is_read, course_id, sender_id, created_at, type) values ("${data.user}","${data.title}","${data.content}", 0, 0, "${data.sender_id}","${time_format}", 1)`)
      })
